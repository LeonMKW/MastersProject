---
title: "Large Data Visualization techniques using decorators and Listdown Package"
author: "Mingkun Wu"
date: "11/26/2020"
output:
...
Statisticians and analysts have been using R (Ihaka, 1998) for a long time. R programming environment has it's formal document standards embedded, such as R markdown. While R markdown generate documents itself programmatically, it requires the programmer to manually construct the structure of a file as a whole reproducible document. For the majority of coding work done as prose contents, namely computational components, the output are the main interest, and is often followed with contexts, defined as narrative components. The __listdown__ Package available on the __Comprehensive R Archive Network (CRAN)__ provides a programmatic solution to generate documents containing computational and narrative components efficiently, including computational components of visualizations. This paper demonstrates multiple methods implanting interactive plotting systems and graphical packages in R to a output document when users generate reproducible documents using the listdown Package. These methods are mainly supported by the ```plotly``` and ```trelliscopeJS``` packages available on CRAN. The concepts are illustrated using the gapminder data set from the ```gapminder``` package, and will be applied to a current analytical task on COVID-19 data sourced from the World Health Organization (https://www.who.int). 
...
R language has been an popular computing language since it's first appearance back in the 90s. R has many features dedicated but not limited to statistical computing, data analysis and other revelant tasks across a greater number of industries and fields. In addition to the IDE (Intergrated Development Enviroment) software R studio, a large number of R packages are also available, intended for users to focus more on the "goal" instead of the "coding". In the rest of chapter 1, the paper discusses how reproducibility and compatibility of multiple statistical tasks can be contained in one single document by using R markdown, followed by data visualizations in R. The rest of this paper mainly focuses on the decorators of the already released __listdown__ package (Kane,Jiang and Urbanek, 2020). While big data visualization tasks has become much more achievable in R with the release of many famous visualization packages, we will try to embed these features into listdown, especially the idea of facetting. The intention will be explained first followed by the methods. Then, the methods and functions are testified. Finally, a case study will be carried out using the methods created. 
...
In this example, the RDS file __comp-comp_covid.rds__ remains the same as the previous example. The data used were also inserted into the listdown object. Now we ca use the datatabe decorator to display the data. From the description of the ld object, the computational components are identical, the only difference tends to be the decorators and it's associated packages imported.
...
While the decorators now works with trelliscope and plotly, there are still some issues that is yet to be solved. Firstly, if there are more than one ggplot objects in a list, both function __facet_wrap2ts__ and __facettsplotly__ only supports turning the first ggplot object into the desired output. This was a problem with the __path__ argument as trelliscope facet components are stored in different directories than __facet_wrap__. Secondly, The are much more interactive visualization packages that are potentially possible to embed to listdown with decorators, such as the __NetworkD3__ package (Allaire, Ellis, Gandrud, Kuo, Lewis, Owen, et al., 2017). The ideas from the trelliscope decorators in this paper has provided a pathway to future decorator functions. Furtherwork could focus on making multiple objects suitable for applying the trellicope decorators in the same list and creating more decorators for the __listdown__ package. 
