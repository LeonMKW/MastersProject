...
### 2.2.2 Case Study: COVID-19 Dataset Example
...
Referring to step 3 in chapter 2.2.1, the decorator is a description of the function from the computational components in the list being created. The decorator Itself is not a function call evaluatable by R, it describes the method of the function to be rendered during the process of knitting. When Knitr renders the R markdown file into a HTML document, the description stated in the decorator are injected to the markdown file telling the knitr to use the decribed method in the decorator instead of the original function arguments. In this case, the first step is to attach the packages required for listdown to render the document from the listdown object. Building the computational components in the list requires library ggplot2, tidyverse and scales, hence they are stated. And because we are using ```ggplotly``` from the plotly package as a decorator, library plotly is also attached. The next step is to replace the ggplot function call with the decorator ggplotly. This means when the listdown objects are rendered, method ggplotly will be used instead of ggplot and will be applied on all ggplot class objects within the list. From here, a simple header will be added and the HTML document will be rendered.  
...
### 3.2.2 Facetting the gapminder Data Set
```{r}
library(listdown)
library(gapminder)
library(trelliscopejs)
...
### 4.2.2 Testing facet_wrap2ts on Gapminder Data set
...
From the previous examples, the trelliscopeJS works especially well with big data set having variables with multiple levels. 
