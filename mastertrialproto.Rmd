...
# covid data with countries using trelliscope include tyding
```{r}
library(trelliscopejs)
library(listdown)
library(tidyverse)
...
`Cases against Death facet` = ggplot(filter(covidcountries, New_cases >= 0)) + 
    geom_point(aes(x = New_cases,y = New_deaths), alpha=.2) +
    scale_x_sqrt() +
    scale_y_sqrt() +
    facet_wrap(vars(WHO_region)) +
    labs(title = "Number of COVID cases against death by WHO region by facet")
